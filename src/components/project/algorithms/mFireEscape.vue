...
class Solution {
    ...
    // Sentinel value for areas not reached by fire
    
    int dirx[4] = {1, 0, -1, 0}; 
    // Directions for row movement (down, left, up, right)
    
    int diry[4] = {0, -1, 0, 1}; 
    // Directions for column movement (down, left, up, right)
    ...
    int maximumMinutes(vector<vector<int>>& grid) {
        ...
        while (!q.empty()) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                if (nx >= 0 && nx < m && ny >= 0 
                    && ny < n && fireTime[nx][ny] == NO_FIRE) {
                    fireTime[nx][ny] = fireTime[x][y] + 1; 
                    // Spread fire to the next cell
                    q.push({nx, ny});
                }
            }
        }
        ...
        auto canEscape = [&](int delay) -> bool {
            ...
            queue<pair<int, int>> q; 
            // Queue for BFS of escape path
            
            q.push({0, 0});
            vis[0][0] = 1;

            int time = delay; 
            // Current time starts with delay
...
public:
    ...
    // Record initial fire positions
        ...
        // Time complexity is O(mn) as it needs to go through 
        // every grid which is m * n
    }
};
...
function initial() {
    screenWidth.value = scrollContainer.value!;
                ...
                <div>
                    ...
                    <n-flex vertical :size="48">
                        <div>
                            ...
                            <grid-display-vue :width="'100%'" :rows="5" :cols="7" :figurePosition="0" :allowSlide="true"
                                :showPlayer="true" :showExit="true" :allowSetup="true"></grid-display-vue>
                        </div>
                        <div>
                            <div ref="firstThought" class="subTitle">First thought about the question</div>
                            <p><b class="list">1.</b>The first word we see in the question is a 2D array, so our first
                                step
                                is to initilaize a 2D array to store grass(0), fire(1), and wall(2) <br>
                                <b class="list">2.</b>The question then say <b class="greyhighlight"> Every minute
                                </b>the
                                player can move to an adjacent <b class="greyhighlight">grass</b> cell. This hints us
                                that
                                we can use <b class="link" @click="scrollToBFS">BFS</b> to solve this question as the
                                player
                                moves step by step while identifying whether each step is valid using the simulation of
                                the
                                spread of fire <br>
                                <b class="list">3.</b>Then, the question saids <b class="greyhighlight">"After your
                                    move,
                                    every fire cell will
                                    spread
                                    to all adjacent cells that are not walls."</b> This also hints us to use BFS to
                                simulate
                                the
                                spread of fire as it can move in every direction it can spread, so it is very suitable
                                to
                                use BFS. <br>
                                <b class="list">4.</b>The question ask us to return <b class="greyhighlight">the maximum
                                    minute the person can
                                    stay</b>
                                In order to make sure it is the maximum minute, we can simulate whether the person is
                                able
                                ...
                                is a <b class="greyhighlight">fast</b>way to find the value and the numbers are all
                                sorted.
                            </p>
                        </div>
                        <div>
                            <div ref="bfsSection" class="subTitle">What is BFS?</div>
                            <p> <b>Introduction: </b> This algorithm starts at the root <b
                                    class="greyhighlight">node</b>
                                ...
                                the
                                queue,
                                then the node is continually removed from the queue, all its unvisited neighboring nodes
                                are
                                accessed,
                                and these neighboring nodes are added to the queue. This process is repeated until the
                                queue
                                is
                                empty,
                            ...
                            <div>BFS can also work in <b class="greyhighlight">graph</b> rather than just grid. A graph
                                ...
                                weight, cost, or distance associated with moving from one vertex to another. This
                                addition
                                allows us to analyze not just the connections but also the efficiency of pathways or
                                routes.
                        ...
                        <div>
                            ...
                            <p> Binary Search is an <b class="greyhighlight">efficient</b> method for finding a specific
                                ...
                                works
                                by
                                repeatedly dividing the search interval <b class="greyhighlight">in half</b>. To begin,
                                you
                                select the <b class="greyhighlight">middle value</b> of
                                the
                                sorted
                                array and compare it to <b class="greyhighlight">the target value</b> you're looking
                                for. If
                                ...
                                of
                                the
                                new, smaller interval and comparing it to the target value. The search ends when either
                                <b class="greyhighlight">the
                                    target
                                    value
                                    is found</b> or the interval becomes <b class="greyhighlight">empty</b>, indicating
                                the
                                value isn't in the array. This
                                approach
                                is
                                analogous to finding a name in a phone book by repeatedly opening it to the middle,
                                deciding
                                whether
                                to
                                ...
                                tool in
                                computer science and everyday problem-solving, particularly when dealing with sorted
                                data.
                            </p>
                            <div>Its time complexity is <b
                                    style="font-size: 1.375rem; font-weight: 2000; font-style: italic; font-family:serif;">O(log(n))</b>
                            </div>
                            <div style="text-align: center;"><b>Imagine you need to find number 2 in an array -5 to
                                    5</b>
                            </div>
                        ...
                        <div>
                            ...
                            <p><b class="list">1. </b>Recording the initial position of the fire. Use a list to record
                                each
                            ...
                            <grid-display-vue :width="'100%'" :rows="5" :cols="7" :figurePosition="0"
                                :allowSlide="false" :showPlayer="true" :showExit="true"
                                :allowSetup="false"></grid-display-vue>
                            ...
                            <div style="align-items: center; text-align: center; padding: auto;"><b>How the fire
                                    spread:</b>
                            </div>
                            <div>
                                <grid-display-vue :width="'100%'" :rows="5" :cols="7" :figurePosition="0"
                            ...
                            <p><b class="list">3. </b>Perform another BFS to check whether you can reach the safehouse
                                starting
                                from
                                the top-left corner, and the time needed (if possible) considering the fire spread at
                                each
                                minute
                                and
